...
int input_get_guess(double *xguess,
  ...
  for (index_guess=0; index_guess < pfzw->target_size; index_guess++) {
    ...
    case Omega_scf:
      /* *
       * ET: In this case keep the default shooting for V0, could be updated with a switch for the other parameters
      ...
      if (ba.use_scf_parameters == _TRUE_) {
        ...
        switch (ba.scf_shooting_target) {
          case scf_shoot_V0: xguess[index_guess] = ba.V0_scf; break;
          case scf_shoot_lambda: xguess[index_guess] = ba.lambda_scf; break;
          ...
          case scf_shoot_phi_prime_ini: xguess[index_guess] = ba.phi_prime_ini_scf; break;
          case scf_shoot_none:
          default:
...
int input_try_unknown_parameters(double * unknown_parameter,
  ...
  for (i=0; i < pfzw->target_size; i++) {
    ...
    case omega_ini_dcdm:
      ...
      break;
    /* ET: Added cases for idm shooting */
    case Omega_idm:
...
int input_read_parameters_species(struct file_content * pfc,
  ...
  class_test(((flag1 == _TRUE_) && (flag2 == _TRUE_)),
             errmsg,
             "You can only enter one of 'Omega_idm' or 'omega_idm'.");
  /* Complete set of parameters */
  if (flag1 == _TRUE_){
  ...
  /* ET: Read Omega_ini_idm or omega_ini_idm */
  class_call(parser_read_double(pfc,"Omega_ini_idm",&param1,&flag1,errmsg),
             errmsg,
             errmsg);
  class_call(parser_read_double(pfc,"omega_ini_idm",&param2,&flag2,errmsg),
             errmsg,
             errmsg);
  class_test(((flag1 == _TRUE_) && (flag2 == _TRUE_)),
             errmsg,
             "In input file, you can only enter one of Omega_ini_idm or omega_ini_idm, choose one");
  if (flag1 == _TRUE_)
    pba->Omega_ini_idm = param1;
  if (flag2 == _TRUE_)
  ...
  if (pba->Omega0_scf != 0.){
    ...
    class_call(parser_read_string(pfc,
                                  "scf_shooting_target",
                                  &string1,
                                  &flag_scf_shoot_target,
                                  errmsg),
               errmsg,
               errmsg);
    if (flag_scf_shoot_target == _TRUE_) {
      if ((strstr(string1,"V0_2") != NULL) || (strstr(string1,"v0_2") != NULL) ||
          (strstr(string1,"scf_V0_2") != NULL) || (strstr(string1,"SCF_V0_2") != NULL)) {
        pba->scf_shooting_target = scf_shoot_V0_2;
      }
      else if ((strstr(string1,"lambda_2") != NULL) || (strstr(string1,"LAMBDA_2") != NULL) ||
               (strstr(string1,"scf_lambda_2") != NULL) || (strstr(string1,"SCF_LAMBDA_2") != NULL)) {
        pba->scf_shooting_target = scf_shoot_lambda_2;
      }
      else if ((strstr(string1,"V0") != NULL) || (strstr(string1,"v0") != NULL) ||
               (strstr(string1,"scf_V0") != NULL) || (strstr(string1,"SCF_V0") != NULL)) {
        pba->scf_shooting_target = scf_shoot_V0;
      }
      else if ((strstr(string1,"lambda") != NULL) || (strstr(string1,"LAMBDA") != NULL) ||
      ...
      else if ((strstr(string1,"alpha") != NULL) || (strstr(string1,"ALPHA") != NULL) ||
               (strstr(string1,"scf_alpha") != NULL) || (strstr(string1,"SCF_ALPHA") != NULL)) {
        pba->scf_shooting_target = scf_shoot_alpha;
      }
      else if ((strstr(string1,"D0") != NULL) || (strstr(string1,"d0") != NULL) ||
               (strstr(string1,"scf_D0") != NULL) || (strstr(string1,"SCF_D0") != NULL)) {
        pba->scf_shooting_target = scf_shoot_D0;
      }
      else if ((strstr(string1,"phi_prime") != NULL) || (strstr(string1,"phi'_") != NULL) || (strstr(string1,"phi_prime_ini") != NULL) ||
               (strstr(string1,"scf_phi_prime") != NULL) || (strstr(string1,"scf_phi_prime_ini") != NULL) ||
               (strstr(string1,"SCF_PHI_PRIME") != NULL) || (strstr(string1,"SCF_PHI_PRIME_INI") != NULL)) {
        pba->scf_shooting_target = scf_shoot_phi_prime_ini;
      }
      else if ((strstr(string1,"phi") != NULL) || (strstr(string1,"PHI") != NULL) ||
               (strstr(string1,"scf_phi") != NULL) || (strstr(string1,"SCF_PHI") != NULL)) {
        pba->scf_shooting_target = scf_shoot_phi_ini;
      }
    ...
    class_call(parser_read_double(pfc,"scf_gamma0",&scf_gamma0_val,&flag_gamma0,errmsg),
               errmsg,
               errmsg);
    class_call(parser_read_double(pfc,"scf_gamma",&scf_gamma_legacy_val,&flag_gamma_legacy,errmsg),
               errmsg,
               errmsg);
    class_call(parser_read_double(pfc,"scf_log10minus_gamma0",&scf_log10minus_gamma0_val,&flag_log10minus_gamma0,errmsg),
               errmsg,
               errmsg);
    class_call(parser_read_double(pfc,"scf_log10minus_gamma",&scf_log10minus_gamma_legacy_val,&flag_log10minus_gamma_legacy,errmsg),
               errmsg,
               errmsg);
    class_test((flag_gamma0 == _TRUE_) && (flag_gamma_legacy == _TRUE_),
               errmsg,
               "You cannot set both scf_gamma0 and scf_gamma. Use scf_gamma0.");
    class_test((flag_log10minus_gamma0 == _TRUE_) && (flag_log10minus_gamma_legacy == _TRUE_),
               errmsg,
               "You cannot set both scf_log10minus_gamma0 and scf_log10minus_gamma. Use scf_log10minus_gamma0.");
    class_test(((flag_gamma0 == _TRUE_) || (flag_gamma_legacy == _TRUE_)) &&
               ((flag_log10minus_gamma0 == _TRUE_) || (flag_log10minus_gamma_legacy == _TRUE_)),
               errmsg,
               "You cannot set both scf_gamma0/scf_gamma and scf_log10minus_gamma0/scf_log10minus_gamma. Choose one.");
    /* ET: explicit entropy-coupling/source inputs */
    class_call(parser_read_double(pfc,"scf_g0",&scf_g0_val,&flag_g0,errmsg),
               errmsg,
               errmsg);
    ...
    if (pba->use_scf_parameters == _TRUE_) {
      ...
      if ((pba->scf_use_momentum == _TRUE_) &&
          ...
          (flag_log10minus_gamma_legacy == _FALSE_) &&
          (pba->scf_parameters_size > scf_entropy_base)) {
        pba->scf_gamma0 = pba->scf_parameters[scf_entropy_base];
        scf_entropy_base += 1;
      }
      /* ET: backward-compatible entropy block appended to scf_parameters */
      if (flag_explicit_entropy == _FALSE_) {
        if (pba->scf_parameters_size > scf_entropy_base + 0) pba->g0_scf = pba->scf_parameters[scf_entropy_base + 0];
    ...
    if (flag_gamma0 == _TRUE_) pba->scf_gamma0 = scf_gamma0_val;
    else if (flag_gamma_legacy == _TRUE_) pba->scf_gamma0 = scf_gamma_legacy_val;
    if (flag_log10minus_gamma0 == _TRUE_) pba->scf_gamma0 = -pow(10.0,scf_log10minus_gamma0_val);
    else if (flag_log10minus_gamma_legacy == _TRUE_) pba->scf_gamma0 = -pow(10.0,scf_log10minus_gamma_legacy_val);
    /* ET: explicit entropy inputs override list/default values */
    if (flag_g0 == _TRUE_) pba->g0_scf = scf_g0_val;
    ...
    if (pba->scf_use_momentum == _TRUE_) {
      ...
      class_test(fabs(1.-2.*pba->scf_gamma0) < 1e-12,
                 errmsg,
                 "For momentum coupling you must avoid scf_gamma0=1/2 (singular 1-2*scf_gamma0 denominator).");
    }
    ...
    if (pba->use_scf_parameters == _TRUE_) {
      ...
      double scf_shoot_val = 0.0;
      int flag_shoot_param = _FALSE_;
      class_call(parser_read_double(pfc,"scf_shooting_parameter",&scf_shoot_val,&flag_shoot_param,errmsg),
                 errmsg,
                 errmsg);
      if (flag_shoot_param == _TRUE_) {
        class_test(pba->scf_shooting_target == scf_shoot_none,
                   errmsg,
                   "When using explicit scf_* inputs, you must set scf_shooting_target if scf_shooting_parameter is used.");
        switch (pba->scf_shooting_target) {
          case scf_shoot_V0: pba->V0_scf = scf_shoot_val; break;
          case scf_shoot_lambda: pba->lambda_scf = scf_shoot_val; break;
          ...
          case scf_shoot_phi_prime_ini: pba->phi_prime_ini_scf = scf_shoot_val; break;
          default: break;
        }
      }
      class_test((pba->Omega0_scf < 0.) && (pba->scf_shooting_target == scf_shoot_none),
                 errmsg,
                 "Omega_scf < 0 requires scf_shooting_target when using explicit scf_* inputs.");
    }
  }
