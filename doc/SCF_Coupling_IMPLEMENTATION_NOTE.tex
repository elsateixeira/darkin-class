...
\texttt{fig3\_2505\_matter.ini}.

The Fig.~3 reproduction uses exactly the parameter values of Eq.~(3.8) of
\texttt{2505.10410v2.pdf}.  With the identification $C_m\equiv A_{m\varphi}$ and
$\beta_m\equiv\alpha_m$ between Eq.~\eqref{eq:amform} and Eq.~(3.7) of the paper,
\beq
\lambda=\sqrt{2},\qquad
C_m=\frac18,\qquad
\beta_m=\sqrt{\tfrac23},\qquad
w_{\varphi0}=-0.76365999931 ,
\eeq
set in the code as \texttt{scf\_lambda = 1.4142135623730951},
\texttt{scf\_C0 = 0.125}, \texttt{scf\_beta = 0.816496580927726}, and
\texttt{scf\_w\_phi\_today = -0.76365999931}.  (The slope is $\sqrt2$, not $2$.)
...
$\rhob_r=\rho_{\rm idr}/\Ar$ and $\rhob_m=\rho_{\rm qcdm}/\Am$.  The middle row
shows the effective dark-energy density $\rhode$ [Eq.~\eqref{eq:deeffrho}]; since
$\rhode$ can change sign where $(\Ar-1)<0$, we plot $\log_{10}|\rhode/H_0^2|$ and
draw the negative stretches with a dotted line.}
\label{fig:radiation}
\end{figure}
